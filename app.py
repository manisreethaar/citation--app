...
def index():
    ...
    return render_template(
        ...
        plag_api_key=_PLAG_API_KEY,
        plag_cse_id=_PLAG_CSE_ID,
    )
...
try:
    import sys as _plag_sys
    _plag_sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'v2'))
...
def check_index():
    return redirect(url_for('index') + '#plagiarism')


@app.route('/run_plagiarism_check', methods=['POST'])
def run_plagiarism_check():
    if not _PLAG_AVAILABLE:
        flash('Plagiarism checker is not available.', 'error')
        return redirect(url_for('index'))
    ...
    doc_path = os.path.join(upload_folder, 'plag_' + secure_filename(doc_file.filename))
    ...
    # Source files
    sources = []
    for sf in request.files.getlist('sources'):
        if sf and sf.filename:
            sp = os.path.join(upload_folder, 'src_' + secure_filename(sf.filename))
            sf.save(sp)
            try:
                sources.append((sf.filename, read_text(sp)))
            except Exception:
                pass

    # Run checks
    engine_result = _plag_check(doc_text, sources)

    web_matches = []
    if request.form.get('web_check') == '1':
        api_key = request.form.get('google_api_key', '').strip() or _PLAG_API_KEY
        cse_id  = request.form.get('google_cse_id', '').strip()  or _PLAG_CSE_ID
        web_matches = _web_check(doc_text, api_key=api_key, cse_id=cse_id)

    report_text = _build_plag_report(
        engine_result, web_matches,
        doc_name=doc_file.filename,
        source_names=[s[0] for s in sources],
    )

    report_name = Path(doc_file.filename).stem + '_plagiarism_report.txt'
    report_path = os.path.join(upload_folder, report_name)
    with open(report_path, 'w', encoding='utf-8') as rf:
        rf.write(report_text)

    zip_name = Path(doc_file.filename).stem + '_plagiarism_check.zip'
    zip_path = os.path.join(upload_folder, zip_name)
    with zipfile.ZipFile(zip_path, 'w') as z:
        z.write(report_path, report_name)
