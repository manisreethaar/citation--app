...
def _to_superscript(s): return s.translate(_SUPER)


# ─── Shared surname extractor ─────────────────────────────────────────────────
...
def _surname(author: str) -> str:
    ...
    if not parts:
        return author
    # "Smith J" or "Smith JK" → "Smith"  (last token is initials)
    if len(parts) >= 2 and re.fullmatch_safe(parts[-1]):
        return parts[0]
    # "John Smith" → "Smith"
    return parts[-1]
...
def _apa_entry(ref: Reference) -> str:
    ...
    if j: parts.append(j)
    if ref.doi: parts.append(ref.doi)
    result = ' '.join(parts)
...
def _vancouver_entry(ref: Reference) -> str:
    ...
    if ref.title:  parts.append(f'{ref.title}.')
    j = _journal_block(ref)
    if j: parts.append(j)
    if ref.doi:
        doi_val = ref.doi if not ref.doi.lower().startswith('doi') else ref.doi
        parts.append(doi_val)
    return ' '.join(parts)
...
def _ieee_entry(ref: Reference) -> str:
    ...
    if ref.title:   parts.append(f'"{ref.title},"')
    if ref.journal: parts.append(f'{ref.journal},')
    if ref.volume:
        v = f'vol. {ref.volume}'
        if ref.issue: v += f', no. {ref.issue}'
        parts.append(v + ',')
    if ref.pages:   parts.append(f'pp. {ref.pages},')
    if ref.year:    parts.append(f'{ref.year}.')
    if ref.doi:
        doi_val = ref.doi if not ref.doi.lower().startswith('doi') else ref.doi
        parts.append(doi_val + '.')
    return ' '.join(parts)
...
def _nature_entry(ref: Reference) -> str:
    ...
    vp = []
    if ref.volume: vp.append(ref.volume)
    if ref.pages:  vp.append(ref.pages)
    if ref.year:   vp.append(f'({ref.year})')
    if vp: parts.append(', '.join(vp) + '.')
    if ref.doi: parts.append(ref.doi)
    return ' '.join(parts)
...
def _nature_author_list(authors: List[str]) -> str:
    ...
    for a in authors[:10]:
        surname, inits = _normalise_author(a)
        inits_dotted = '.'.join(c for c in inits.replace(' ', '').replace('.', '')) + '.' if inits else ''
        fmt.append(f'{surname}, {inits_dotted}' if inits_dotted else surname)
    result = ', '.join(fmt)
    if len(authors) > 10: result += ' et al.'
    return result
...
def _journal_block(ref: Reference) -> str:
    ...
    if ref.volume:
        v = ref.volume
        if ref.issue: v += f'({ref.issue})'
        parts.append(v)
    if ref.pages:   parts.append(ref.pages)
